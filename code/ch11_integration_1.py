...
# ある列をスコアとした場合の、テストデータ上の平均nDCGを計算し表示する関数
...
# ドキュメント（ここでは製品タイトル）をトークナイズする関数。トークンのリストにする。
def tokenize_document(text):
    return FUGASHI_TAGGER.parse(text).split()


# キーワード検索の特徴量として、クエリ中のトークンが製品タイトル中に出現する回数 (TF,
# Term Frequency) と、クエリ・製品タイトルそれぞれの長さ（トークン数）を計算する関数
...
def train_xgb_ranker(data, features):
    ...
    xgb_ranker = xgb.XGBRanker(
        n_estimators=8, max_depth=2, objective="rank:ndcg", ndcg_exp_gain=False
    )
...
# Reciprocal Rank Fusion (RRF) で順位に加算する定数
RRF_K = 60
